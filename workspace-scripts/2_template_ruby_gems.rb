...
def run_kettle_jem_for_gem(gem_dir, options)
  ...
  result = Kettle::Jem::Tasks::InstallTask.run(project_root: gem_dir, env: env, run_options: run_options)
...
def normalize_lockfile_for_gem(gem_dir)
  ...
  bundle_action = File.exist?(File.join(gem_dir, "Gemfile.lock")) ? %w[update --bundler] : %w[install]
  command = ["mise", "exec", "-C", gem_dir, "--", "env", *command_env, "bundle", *bundle_action]
