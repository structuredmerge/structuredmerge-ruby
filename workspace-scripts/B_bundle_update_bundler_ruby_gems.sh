...
bundle_update_bundler_gem() {
  ...
  mise exec -C "$dir" -- bundle update --bundler
  UPDATED_LOCKFILES+=("$dir/Gemfile.lock")
  echo ""
}

run_for_selected_gems bundle_update_bundler_gem
commit_selected_lockfiles "📌 Upgrade bundler" "${UPDATED_LOCKFILES[@]}"
echo "=== ALL DONE ==="
