...
module Kettle
  module Jem
    ...
    #   DEFAULT_ENGINES) and has no template-managed modular home (below the
    #   template's inclusion threshold). Detected by #engine_dependency_conflicts, a
    ...
    KNOWN_GEM_CONFLICT_RESOLUTIONS = {
      ...
      "sqlite3" => {
        ...
        action: "review",
        reason: "sqlite3 has no jruby-platform release on rubygems.org and is not template-managed; declare it in a modular gemfile with platforms: [:ruby] instead of a direct dependency (kettle-jem-appraisals generates gemfiles/modular/activerecord_support/*.gemfile, with the JDBC adapter on JRuby), or resolve per destination based on declared engines.",
    ...
    # below the template's inclusion threshold with no template-managed
    # modular home (sqlite3 is the pilot; see KNOWN_GEM_CONFLICT_RESOLUTIONS), so the
    ...
    def github_actions_framework_matrix(config)
      ...
      gemfiles = version_entries.map do |entry|
        ...
        gemfile_entry.merge!(framework_matrix_security(framework_gem, entry))
        gemfile_entry
    ...
    # Security data from kettle-rb (Kettle::Rb::GemFloors) for a framework
    # matrix version of a tracked gem. The series' security floor is added
    # when the version's requirement still admits the series' first release,
    # and advisories that no release in the series fixes are recorded so the
    # generated gemfile can warn about them.
    def framework_matrix_security(gem_name, entry)
      return {} unless Kettle::Rb::GemFloors.tracked?(gem_name)

      series = Kettle::Rb::GemFloors.series_for(entry.fetch(:label))
      return {} unless series

      security = {}
      first_release = Gem::Version.new("#{series}.0")
      floor = Kettle::Rb::GemFloors.floor(gem_name, series)
      requirement = Gem::Requirement.new(*entry.fetch(:requirement).split(/\s*,\s*/))
      if floor && Gem::Version.new(floor) > first_release && requirement.satisfied_by?(first_release)
        security[:security_floor] = floor
      end
      unpatched = Kettle::Rb::GemFloors.unpatched_advisories(gem_name, series)
      security[:unpatched_advisories] = unpatched unless unpatched.empty?
      security[:security_series] = series unless security.empty?
      security
    rescue ArgumentError
      {}
    end
    ...
    def synchronize_github_actions_framework_gemfile(target_path, facts)
      ...
      lines << ""
      unpatched = gemfile.fetch(:unpatched_advisories, [])
      unless unpatched.empty?
        lines << "# kettle-rb: no #{gemfile.fetch(:gem)} #{gemfile.fetch(:security_series)} release fixes #{unpatched.join(", ")}"
      end
      requirements = [gemfile.fetch(:requirement)]
      requirements << ">= #{gemfile.fetch(:security_floor)}" if gemfile[:security_floor]
      lines << %(gem #{[gemfile.fetch(:gem), *requirements].map(&:inspect).join(", ")})
      "#{lines.join("\n")}\n"
    end
