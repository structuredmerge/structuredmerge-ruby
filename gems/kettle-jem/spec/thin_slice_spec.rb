...
RSpec.describe Kettle::Jem do
      ...
      write_tree(root, {
        "example-gem.gemspec" => <<~RUBY,
          Gem::Specification.new do |spec|
            ...
            spec.version = "1.2.3"
            spec.summary = "Example gem"
            spec.add_dependency "version_gem", "~> 1.1", ">= 1.1.10"
          end
        RUBY
        ".kettle-jem.yml" => <<~YAML,
          project_emoji: "💎"
  ...
  it "templates a monorepo root without a gemspec and syncs root Gemfile tooling dependencies" do
    ...
    Dir.mktmpdir("kettle-jem-monorepo-root-apply", tmp_root) do |root|
      ...
      expect(gemfile).to include('gem "kettle-dev", "~> 2.1", ">= 2.1.0"')
      ...
      Dir.mktmpdir("kettle-jem-install-monorepo", tmp_root) do |repo_root|
        ...
        binstub_runner = lambda do |command, chdir:, env:, quiet:|
          ...
          if command == curated_binstubs
            ...
            File.write(File.join(chdir, "bin", "appraisal"), "#!/usr/bin/env ruby\nputs 'appraisal binstub'\n")
            File.write(File.join(chdir, "bin", "rake"), "#!/usr/bin/env ruby\nputs 'rake binstub'\n")
            File.write(File.join(chdir, "bin", "yard"), "#!/usr/bin/env ruby\nload Gem.bin_path(\"yard\", \"yard\")\n")
            File.write(File.join(chdir, "bin", "reek"), <<~RUBY)
              #!/usr/bin/env ruby
  ...
  it "keeps the greater version requirement for template-managed gemspec dependencies" do
    ...
    Dir.mktmpdir("kettle-jem-gemspec-template-managed-dependencies", tmp_root) do |root|
      write_tree(root, {
        ...
        "template/example.gemspec.example" => <<~RUBY,
          Gem::Specification.new do |spec|
            ...
            spec.add_development_dependency("kettle-dev", "~> 2.1", ">= 2.1.0")
            spec.add_development_dependency("rake", "~> 13.0")
          end
        RUBY
      })
      ...
      expect(gemspec_content).to include(%(spec.add_development_dependency("kettle-dev", "~> 2.1", ">= 2.1.0")))
  ...
  it "projects RuboCop LTS template tokens from minimum Ruby" do
    ...
    Dir.mktmpdir("kettle-jem-rubocop-token-slice", tmp_root) do |root|
      ...
      expect(template_report.fetch(:final_content)).to include('gem "appraisal2-rubocop", "~> 0.1", ">= 0.1.0", require: false')
