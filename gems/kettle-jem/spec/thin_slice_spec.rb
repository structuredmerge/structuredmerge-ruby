...
RSpec.describe Kettle::Jem do
      ...
      write_tree(root, {
        "example.gemspec" => <<~RUBY,
          Gem::Specification.new do |gem|
            ...
            gem.add_dependency("kettle-test", "~> 2.0", ">= 2.0.2")
          end
        RUBY
        ".kettle-jem.yml" => <<~YAML,
          templates:
        ...
        "template/example.gemspec.example" => <<~RUBY,
          Gem::Specification.new do |spec|
            ...
            spec.add_development_dependency("kettle-test", "~> 2.0", ">= 2.0.2")
            spec.add_development_dependency("rake", "~> 13.0")
          end
        RUBY
      })
  ...
  it "projects RuboCop LTS template tokens from minimum Ruby" do
    ...
    Dir.mktmpdir("kettle-jem-rubocop-token-slice", tmp_root) do |root|
      ...
      expect(template_report.fetch(:final_content)).to include('gem "rubocop-ruby3_1", "~> 3.0.1"')
      expect(template_report.dig(:metadata, :template_tokens)).to include(
        "KJ|RUBOCOP_LTS_CONSTRAINT" => "~> 22.2.0",
        "KJ|RUBOCOP_RUBY_CONSTRAINT" => "~> 3.0.1",
