...
RSpec.describe TypeScript::Merge do
  ...
  it 'conforms to the TypeScript source-family fixtures' do
    feature = read_json(fixtures_root.join('diagnostics', 'slice-101-typescript-family-feature-profile',
    ...
    backends_fixture = read_json(fixtures_root.join('diagnostics', 'slice-115-typescript-family-backends',
                                                    'typescript-backends.json'))
    expect(described_class.available_type_script_backends.map(&:to_h)).to eq([{ id: 'kreuzberg-language-pack',
                                                                                family: 'tree-sitter' }])
    ...
    canonical_manifest = read_json(fixtures_root.join('conformance', 'slice-24-manifest',
