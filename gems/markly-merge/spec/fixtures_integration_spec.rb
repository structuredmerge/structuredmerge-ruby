...
RSpec.describe Markly::Merge do
  ...
  it "projects the structured-edit request through Markly" do
    fixture = read_json(
      fixtures_root.join(
        "diagnostics",
        "slice-429-markdown-structured-edit-request-projection",
        "markdown-structured-edit-request-projection.json"
      )
    )

    expect(json_ready(described_class.markdown_structured_edit_request_projection)).to eq(
      json_ready(fixture.dig(:providers, :markly))
    )
  end

  it "projects the structured-edit result through Markly" do
    fixture = read_json(
      fixtures_root.join(
        "diagnostics",
        "slice-431-markdown-structured-edit-result-projection",
        "markdown-structured-edit-result-projection.json"
      )
    )

    expect(json_ready(described_class.markdown_structured_edit_result_projection)).to eq(
      json_ready(fixture.dig(:providers, :markly))
    )
  end

  it "projects the structured-edit application through Markly" do
    fixture = read_json(
      fixtures_root.join(
        "diagnostics",
        "slice-434-markdown-structured-edit-application-projection",
        "markdown-structured-edit-application-projection.json"
      )
    )

    expect(json_ready(described_class.markdown_structured_edit_application_projection)).to eq(
      json_ready(fixture.dig(:providers, :markly))
    )
  end
  ...
  it "conforms to the provider named-suite plan and manifest-report fixtures" do
    ...
    contexts = plans_fixture.dig(:contexts, :markly)
    expect(json_ready(Ast::Merge.plan_named_conformance_suites(plans_fixture[:manifest], contexts))).to eq(
      json_ready(plans_fixture.dig(:expected_entries, :markly))
    )

    executions = report_fixture[:executions]
    entries = Ast::Merge.report_planned_named_conformance_suites(
      Ast::Merge.plan_named_conformance_suites(report_fixture[:manifest], report_fixture.dig(:options, :markly, :contexts))
    ) do |run|
    ...
    expect(json_ready(Ast::Merge.report_named_conformance_suite_envelope(entries))).to eq(
      json_ready(report_fixture.dig(:expected_reports, :markly))
    )
  end
end
