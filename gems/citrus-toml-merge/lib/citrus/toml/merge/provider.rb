...
module Citrus
  module Toml
    ...
    module Merge
      # Conservative source-preserving provider for top-level TOML mappings.
      ...
      class Provider
        ...
        def provider_id = 'ruby.toml.citrus'
        def family = 'toml'
        ...
        def analyze_role(role, source)
          analysis = TreeHaver.with_backend(BACKEND.id) { ::Toml::Merge::FileAnalysis.new(source) }
        ...
        def envelope(request, **fields)
          {
            provider: {
              ...
              backend: request[:backend] || :citrus,
              package: PACKAGE_NAME,
              package_version: Citrus::Toml::Merge::Version::VERSION
            },
