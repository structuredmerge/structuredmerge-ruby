...
RSpec.describe Citrus::Toml::Merge do
  def fixtures_root
  ...
  it 'exposes the TOML family through the Citrus provider backend' do
    ...
    expect(TreeHaver.registered_languages(:toml)).to include(:citrus)
    expect(
      TreeHaver.with_backend('citrus') { TreeHaver.parser_for(:toml).parse('title = "x"').root_node.type }
    ).to eq('document')
    ...
    expect(json_ready(described_class.toml_plan_context)).to eq(
      json_ready(plan_fixture.dig(:providers, :citrus))
    )
  end
  ...
  it 'conforms to the provider named-suite plan and manifest-report fixtures' do
    ...
    expect(json_ready(Ast::Merge.report_named_conformance_suite_envelope(entries))).to eq(
      json_ready(report_fixture.dig(:expected_reports, :citrus))
    )
  end
end
