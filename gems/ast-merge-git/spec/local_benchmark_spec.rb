...
RSpec.describe Ast::Merge::Git::LocalBenchmark do
  ...
  it 'validates the exact canonical authored corpus and inline digests' do
    ...
    expect(benchmark.cases.length).to eq(22)
    expect(benchmark.document.fetch('expected_summary')).to include(
      'case_count' => 22,
      'partition_counts' => { 'sentinel' => 14, 'gold' => 6, 'metamorphic' => 2 },
      'operation_counts' => { 'merge2' => 6, 'merge3' => 14, 'metamorphic' => 2 }
    )
  ...
  it 'records complete Ruby golden-master adapter and parser provenance in every run manifest' do
    ...
    manifest.dig('configuration', 'cases').each do |case_configuration|
      family = case_configuration.dig('selector', 'family')
      expect(case_configuration).to include(
        'merge_provider' => include(
          'provider_id' => "ruby.#{family}",
          'require_path' => "#{family}/merge"
        ),
  ...
  it 'records a cross-runtime adapter without attributing it to the Ruby golden master' do
    ...
    descriptor_path.binwrite(
      JSON.generate(
        ...
        'supports' => {
          ...
          'families' => %w[go json markdown rbs ruby toml typescript yaml],
          'dialects' => %w[go json jsonc json5 markdown rbs ruby toml typescript tsx yaml],
          'combinations' => [
    ...
    manifest.dig('configuration', 'cases').each do |configuration|
      provider_id = "rust.#{configuration.dig('selector', 'family')}"
  ...
  it 'measures repeated requests in one persistent process without classifying quality' do
    ...
    expect(performance.fetch('samples').length).to eq(28)
