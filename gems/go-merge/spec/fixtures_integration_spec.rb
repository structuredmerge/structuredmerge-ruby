...
RSpec.describe Go::Merge do
  ...
  it 'conforms to the Go source-family fixtures' do
    feature = read_json(fixtures_root.join('diagnostics', 'slice-109-go-family-feature-profile',
    ...
    expect(described_class.available_go_backends.map(&:to_h)).to eq([{ id: 'kreuzberg-language-pack',
                                                                       family: 'tree-sitter' }])
