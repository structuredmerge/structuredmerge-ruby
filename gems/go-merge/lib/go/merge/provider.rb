...
module Go
  module Merge
    # Base-aware, source-preserving provider for native Go declarations.
    # Parenthesized declarations are whole owners only while their AST-derived
    # member identities remain stable across all three revisions.
    ...
    class Provider
      ...
      def decide(documents, include_unmanaged_conflict:)
        ...
        if include_unmanaged_conflict
          ...
          append_package_conflict(changes, conflicts, documents)
        end
      ...
      def append_unmanaged_conflict(changes, conflicts, documents)
        ...
        changes << change
        conflicts << {
          conflict_id: "go-unmanaged-#{Digest::SHA256.hexdigest([base, ours, theirs].join("\0"))[0, 16]}",
      ...
      def append_package_conflict(changes, conflicts, documents)
        signatures = documents.transform_values do |document|
          document.owners.find { |owner| owner.signature.first == :package }&.signature
        end
        return if signatures.values.uniq.length == 1

        change = { path: '<package-clause>', ours: :unsafe, theirs: :unsafe }.freeze
        changes << change
        conflicts << {
          conflict_id: "go-package-#{Digest::SHA256.hexdigest(signatures.values.join("\0"))[0, 16]}",
          category: :package_identity_change,
          path: change.fetch(:path),
          owner_id: nil,
          change_classification: change
        }.freeze
      ...
      def grouped_ownership_failure(request, documents)
        ...
        conflict = {
          conflict_id: "go-grouped-#{signature_digest[0, 16]}",
          category: :unproven_grouped_ownership,
          path: '<grouped-declarations>',
          owner_id: nil,
        ...
        failure(
          ...
          request,
          category: :unproven_grouped_ownership,
          message: 'Grouped Go declaration membership differs across revisions; whole-owner identity is unproven.',
          conflicts: [conflict],
          ...
          fallbacks: [{
            from: :grouped_declaration_ownership,
            to: :full_file_conflict,
            reason: :grouped_member_identity_changed
          }]
        )
      end
      ...
      def conflict_for(id, base, ours, theirs, change)
        {
          conflict_id: "go-declaration-#{id[0, 16]}",
      ...
      def render_exact(request, documents, role)
        ...
        unless verification[:semantic_match] && verification[:byte_exact]
          return failure(
            ...
            message: 'Go exact revision did not remain byte-exact and semantically identical after native reparse.',
            source_role: role,
      ...
      def render_composite(request, documents, decision)
        ...
        unless verification[:semantic_match]
          return failure(
            ...
            request,
            category: :render_failure,
            message: 'Go composite did not match the planned ordered declaration structure and AST attributes.',
      ...
      def append_declaration_additions(fragments, documents, decision)
        ...
        additions.reject! { |owner| owner.signature.first == :package }
        return if additions.empty?
      ...
      def grouped_owner?(owner)
        owner.signature.first.to_s.end_with?('_group')
      end

      def import_owner?(owner)
        %i[import import_group].include?(owner.signature.first)
      end

      def package_or_import_owner?(owner)
        owner.signature.first == :package || import_owner?(owner)
      end
      ...
      def unsafe_document_failure(request, role, parsed)
        ...
        conflict_id = "go-unsafe-#{Digest::SHA256.hexdigest([role, failure_detail(parsed)].join("\0"))[0, 16]}"
        conflict = {
      ...
      def conflict_failure(request, decision, rendered, strategy, fallbacks: [])
        failure(
          ...
          message: 'Go top-level declaration changed incompatibly on both sides.',
          changes: decision.changes,
