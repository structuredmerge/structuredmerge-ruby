...
module Ast
  module Merge
    # Shared typed transport adaptation. Subclasses select explicit Rust profiles.
    class TypedCoreProvider
      ...
      def execute(operation, request)
        unless capabilities[:operations].include?(operation)
          return Ast::Merge::ProviderResult.unsupported(operation: operation, message: 'Typed profile does not implement this operation')
        end
        ...
        typed_request = core::OperationRequest.new(schema: 'structuredmerge.operation-request/v1',
          request_id: request[:request_id] || "ruby-#{provider_id}-#{operation}", operation: policy(operation, request), sources: sources,
          provider_selection: core::MergeProviderSelection.new(provider_id: self.class::CORE_PROVIDER, family: family,
            dialect: dialect, profile_id: self.class::CORE_PROFILE, required_capabilities: [operation.to_s], extra: {}),
      ...
      rescue KeyError, ArgumentError, TypeError, RangeError => e
      ...
      def project(operation, request, result)
        ...
        payload = { output: result.output, conflicted_output: result.conflicted_output, typed_result: result }
        if result.analysis
